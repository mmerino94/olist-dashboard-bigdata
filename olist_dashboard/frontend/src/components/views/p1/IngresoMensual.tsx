...
export default function IngresoMensual() {
  ...
  // Cross-filtering: clic en un mes filtra todo.
  ...
  const option = {
    ...
    xAxis: {
      ...
      axisLine: { lineStyle: { color: "#e3e2e8" } }, axisLabel: { show: false }, axisTick: { show: false },
    },
    ...
    series: [
      {
        type: "line", data: serie.map((r) => r.ingreso), smooth: true, symbol: "circle", symbolSize: 7,
        ...
        areaStyle: { color: { type: "linear", x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: "rgba(60,120,187,0.20)" }, { offset: 1, color: "rgba(60,120,187,0.02)" }] } },
      },
    ],
  };
  ...
  return (
    <div className="bg-paper border border-gray-200 rounded-lg p-5">
      <div className="font-semibold text-ink text-[15px]">
        ...
        <span className="text-[12px] text-blue-accent font-normal">· clic en un mes para filtrar</span>
      </div>
      {loading || error || serie.length === 0 ? (
        <div className="flex items-center justify-center text-sm font-mono text-gray" style={{ height: 240 }}>
          {loading ? "Cargando…" : error ? `Error: ${error}` : "Sin datos para este filtro"}
        </div>
      ) : (
        <>
          <ReactECharts option={option} style={{ height: 220 }} notMerge onEvents={{ click: onClick }} />
